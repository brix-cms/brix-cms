...
public class BrixNodeResource implements IResource {
// ------------------------------ FIELDS ------------------------------

    public static final String SAVE_PARAMETER = Brix.NS_PREFIX + "save";

    private static final Logger log = LoggerFactory.getLogger(BrixNodeResource.class);
    private final IModel<BrixNode> node;
    private final Boolean save;

// --------------------------- CONSTRUCTORS ---------------------------

    public BrixNodeResource(IModel<BrixNode> node) {
        super();
        this.node = node;
        this.save = null;
    }

    public BrixNodeResource(IModel<BrixNode> node, boolean save) {
        super();
        this.node = node;
        this.save = save;
    }
...
// --------------------- Interface IRequestTarget ---------------------
    ...
    public void respond(Attributes attributes) {
        boolean save = (this.save != null) ? this.save : Strings.isTrue(RequestCycle.get().getRequest()
                .getRequestParameters().getParameterValue(SAVE_PARAMETER).toString());
