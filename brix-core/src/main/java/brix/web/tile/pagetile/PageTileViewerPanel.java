...
public class PageTileViewerPanel extends Panel
{
    ...
    public PageTileViewerPanel(String id, IModel<JcrNode> tileNode)
    {
        super(id, tileNode);
    ...
    private void init()
    {
        ...
        else
        {
            ...
            JcrNode pageNode = tileNode.hasProperty("pageNode") ? tileNode.getProperty("pageNode")
                    .getNode() : null;

            if (pageNode != null)
            {
                add(new TilePageRenderPanel("view", new BrixNodeModel(pageNode),
                        (BrixNodeWebPage)getPage()));
            }
            else
            {
