...
{
	/**
	 * Returns list of parent nodes for given node. The list is sorted ascending
	 * by node depth.
	 * 
	 * @param node
	 * @return
	...
	/**
	 * Ensures all nodes from originalNode parent hierarchy exist as children of
	 * targetRootNode. Missing nodes will be created having same primary node
	 * type as original node.
	 * <p>
	 * Example: If originalNode is /foo/bar/baz/node and targetRootNode is /x/y
	 * having child /x/y/foo , this method will create nodes x/y/foo/bar and
	 * x/y/foo/bar/baz .
	 * 
	 * @param originalNode
	...
	/**
	 * Method checks if given value is of type reference and references node
	 * with UUID that has been remapped (can happen with
	 * {@link ImportUUIDBehavior#IMPORT_UUID_CREATE_NEW} being set.
	 * 
	 * @param value
	 * @param uuidMap
	 * @param valueFactory
	...
	/**
	 * Goes through each pair of the node list and copies the properties from
	 * originalNode to targetNode
	 * 
	 * @param nodes
	 * @param uuidMap
	 */
	...
	/**
	 * Creates a copy of originalNode (without setting the properties). The node
	 * position will be concatenation of targetRootNode path and originalNode
	 * path. If the targetRootNode path doesn't contains the appropriate child
	...
	/**
	 * Filters out nodes that are redundantly in the list because their parent
	 * nodes are also in the list.
	 * 
	 * @param nodes
	 * @return
	 */
	...
	/**
	 * Checks if the node referenced by the value is either a child node of a
	 * node from paths or the node exists in targetWorkspace (if targetWorkspace
	 * is not null).
	 * 
	 * @param value
	 * @param node
	...
	/**
	 * Scans the given list of nodes and their children for references that
	 * target nodes outside subtrees of the nodes in the list. Alternatively, if
	 * the referenced node is not part of any subtree and targetWorkspace is not
	 * null, the targetWorkspace is checked for node with same uuid as the
	 * referenced node.
	 * 
	 * @param nodes
	 * @param targetWorkspace
