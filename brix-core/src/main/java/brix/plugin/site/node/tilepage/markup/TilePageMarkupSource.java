...
public class TilePageMarkupSource implements MarkupSource
{
	...
	{
		Date current = null;
		for (TileContainerNode node = this.node; node != null; node = (TileContainerNode) node.getTemplate())
		{
	...
	private void processTag(List<TileContainerNode> nodes, int current, List<Item> items, org.htmlparser.Tag tag)
	{
		final Tag.Type type;
	...
	private void processBrixTag(List<TileContainerNode> nodes, int current, List<Item> items, String tagName,
			Map<String, String> attributes, Tag.Type type)
	{
		TileContainerNode node = nodes.get(current);
	...
	private Map<String, String> getAttributes(org.htmlparser.Tag tag)
	{
		...
		List<Attribute> list = new ArrayList<Attribute>((Collection<? extends Attribute>) original.subList(1, original
				.size()));
